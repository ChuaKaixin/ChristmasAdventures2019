...
public class ComputationHandler15 {
	...
	private static int[] plotRobot(int[][]map, int currentX, int currentY, int previousMove, int moveResult) {
		int[] robotInfo = new int[3];
		/**
		 * 0:new X
		 * 1:new Y
		 * 2:next move
		...
		if(moveResult == 0) {
			switch(previousMove) {
				case 1://north
					markWall(map, currentY+1, currentX);
					break;
				case 2://south
					markWall(map, currentY-1, currentX);
					break;
				case 3://east
					markWall(map, currentY, currentX+1);
					break;
				case 4://west
					markWall(map, currentY, currentX-1);
					break;
	...
	private static void markWall(int[][]map, int x, int y) {
		map[y][x] = 9;
	}
